...
struct throw_on_error
{
	...
	operator() (U const val) const
	{
		if RANT_UNLIKELY(RANT_LESS(T, U, RANT_VALUE(Max), val)) {
			throw RANT_OVERFLOW_ERROR(val, RANT_VALUE(Max));
		}
		return val;
	}
...
struct clip_on_error
{
	...
	{
		return RANT_LESS(T, U, val, RANT_VALUE(Min)) ? RANT_VALUE(Min) :
			RANT_LESS(T, U, RANT_VALUE(Max), val) ? RANT_VALUE(Max) : val;
	}
	...
	operator() (U const val) const
		RANT_IS_NOTHROW_DEFAULT_CONSTR(T)
	{
		return RANT_LESS(T, U, RANT_VALUE(Max), val) ? RANT_VALUE(Max) : val;
	}
};
...
struct wrap_on_error
{
		...
		RANT_IS_NOTHROW_DEFAULT_CONSTR(T)
	{
		...
		if (RANT_LESS(T, U, val, RANT_VALUE(Min))) {
			return RANT_VALUE(Max) - ((RANT_VALUE(Min)-val-1)%diff);
		} else if (RANT_LESS(T, U, RANT_VALUE(Max), val)) {
