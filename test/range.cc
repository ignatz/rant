...
{
	typedef floating_point_range<double, boost::ratio<10>, boost::ratio<0> > t;
...
TYPED_TEST(AllIntegers, Native)
{
	...
	type max = std::numeric_limits<type>::max()-1;
	type min = std::numeric_limits<type>::min()+1;
...
TYPED_TEST(AllIntegers, Limit)
{
	typedef typename TestFixture::type type;
	typedef integral_range<type> t;
	typedef typename boost::conditional<boost::is_unsigned<type>::type::value,
			unsigned long,
			long>::type limit_t;

	limit_t max = std::numeric_limits<type>::max()-1;
	limit_t min = std::numeric_limits<type>::min()+1;
	ASSERT_NO_THROW(t a(max); UNUSED(a););
	ASSERT_NO_THROW(t a(min); UNUSED(a););
	//ASSERT_FALSE(noexcept(t(max)));
}
...
TYPED_TEST(AllFloats, Native)
{
	...
	if (!boost::is_same<type, long double>::type::value) {
		type max =  std::numeric_limits<boost::intmax_t>::max();
		type min = -std::numeric_limits<boost::intmax_t>::max();
		//std::cout.precision(1000);
		//std::cout << std::numeric_limits<t>::max() << " " << max << std::endl;
